        ...
        case HTTP_EVENT_ON_DATA:
            ...
            if (!esp_http_client_is_chunked_response(evt->client)) {
                if (!real_time_set) {
                    char *data = (char *) malloc(evt->data_len + 1);
                    strncpy(data, (char *) evt->data, evt->data_len);
                    data[evt->data_len] = '\0'; // Corrected index
                    // Handle data as needed, for example, send it via MQTT
                    // Assuming you have an MQTT client handle named 'client'
                    int msg_id = esp_mqtt_client_publish(client, "/topic/data", data, 0, 1, 0);
                    ESP_LOGI(TAG, "Published data via MQTT, msg_id=%d", msg_id);
                    free(data);
                }
            }
            break;
...
//         // xTaskCreate(publish_random_numbers, "publish_random_numbers", 4096, NULL, 5, NULL);
//         break;
